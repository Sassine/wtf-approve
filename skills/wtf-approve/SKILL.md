...
Minimal by default. Detail on demand via /wtf:explain.
...
To override: /wtf:language [code]. Accepts any standard format (en, en-US, pt, pt-BR, es, fr, de, ja, etc.).
...
2. ALWAYS consolidate. Never bullet points, numbered lists, or per-command breakdown. /wtf:explain exists for that.
3. Batch risk = highest risk command in the batch.
4. Plain text only. No markdown bold, italic, or backticks.
5. Use the active language (auto-detected or set via /wtf:language). Never translate the original command.
...
```
wtf-approve disabled.
```

### /wtf:on

Re-enables the explainer after /wtf:off.
